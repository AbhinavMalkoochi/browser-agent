#!/usr/bin/env python3
"""
Agent with Anthropic Backend

Demonstrates using the Agent class with a real Anthropic Claude backend.

Prerequisites:
- Chrome must be running with debugging enabled:
  python -m browser_agent.launch_chrome
- Set ANTHROPIC_API_KEY environment variable
- Install anthropic: pip install browser-agent[anthropic]
...
from browser_agent import Agent, AgentConfig, BrowserConfig, AnthropicBackend
...
async def main():
    ...
    print("=== Browser Agent with Anthropic Claude ===\n")
    
    # Create the Anthropic backend
    backend = AnthropicBackend(
        model="claude-3-5-haiku-latest",  # Use claude-sonnet-4-20250514 for better results
        temperature=0.0,
    )
